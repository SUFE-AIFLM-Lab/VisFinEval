...
def setup_logging(config):
    ...
    log_file = log_dir / f'L1_Q4_{timestamp}.log'
...
def analyze_images(pipe, image_paths, question, prompt_template="""作为一位专业的金融数据分析专家,请基于图表严谨地分析以下判断题。
...
def analyze_images_api(client, image_paths, question, model_name, prompt_template="""作为一位专业的金融数据分析专家,请基于图表严谨地分析以下判断题。
...
问题：{question_with_options}
模型输出答案：{model_answer}
正确答案：{correct_answer}

请仔细分析模型输出的内容，判断答案是否实质相同。
请仅输出True或False，True表示答案实质相同，False表示答案不同。"""
    ...
    for result in results:
        ...
        if "error" not in result:
            ...
            # 构建完整的问题字符串
            question_with_options = f"问题：{result.get('question')}"
            logging.info(f"模型输出答案: {model_answer}")
            logging.info(f"正确答案: {correct_answer}")
            
            if model_answer and correct_answer:
                try:
                    is_correct = evaluate_answer(client, model_answer, correct_answer, question_with_options)
...
def load_data(input_path):
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            # 添加到数据列表
            data.append({
                'type': row.get('type', ''),  # 使用get方法提供默认值
                'index': row.get('index', ''),
                'image': row.get('image', ''),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt_template', type=str, 
                       default='作为一位专业的金融分析师，请根据我提供的图片回答以下问题，只需回答是或否即可\n',
                       help='提示模板')
