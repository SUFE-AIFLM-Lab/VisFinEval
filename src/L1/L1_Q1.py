...
def analyze_images(pipe, image_paths, question, prompt_template="""你是一位专业熟悉金融图表并拥有广泛金融和财务知识的金融学专家，我将提供给你一道三选的单项选择题。请回答此问题。
...
def analyze_images_api(client, image_paths, question, model_name, prompt_template="""你是一位专业熟悉金融图表并拥有广泛金融和财务知识的金融学专家，我将提供给你一道三选的单项选择题。请回答此问题。
...
def load_data(input_path):
    ...
    try:
        with open(input_path, 'r', encoding='utf-8') as f:
            ...
            # 将每一行数据添加到列表中，使用get方法安全地获取字段值
            for row in reader:
                data_entry = {
                    'type': row.get('type', ''),  # 如果没有该字段则返回空字符串
                    'index': row.get('index', ''),
                    'image': row.get('image', ''),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt_template', type=str, 
                       default='''你是一位专业熟悉金融图表并拥有广泛金融和财务知识的金融学专家，我将提供给你一道三选的单项选择题。请回答此问题。
...
4. 题目为单选题，请只输出一个选项。
    ...
    try:
        ...
        if not config['model'].get('use_api', False):
            ...
            if "Phi-3.5-vision" in config['model']['path']:
                backend_cfg = PytorchEngineConfig()
            else:
                if "llava-v1.6" in config['model']['path']:
