...
D. {entry['D']}"""
            ...
            # 构建完整的 prompt
            full_prompt = f"{config['evaluation']['prompt_template']}\n{full_question}"
...
def load_data(input_path):
    ...
    try:
        with open(input_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                # 处理image字段，将逗号分隔的多个图片地址转换为列表
                image_paths = [path.strip() for path in row.get('image', '').split(',') if path.strip()]
                
                data.append({
                    'type': row.get('type', ''),
                    'index': row.get('index', ''),
                    'image': image_paths,  # 现在是列表格式
...
请给出你的答案：''',
    ...
    parser.add_argument('--gpu_ids', type=str, default='0,1', help='指定使用的GPU ID，用逗号分隔')
    ...
    try:
        ...
        if not config['model'].get('use_api', False):
            ...
            backend_cfg = TurbomindEngineConfig(tp=config['model']['tp'],cache_max_entry_count = 0.6,session_len = 13000)
